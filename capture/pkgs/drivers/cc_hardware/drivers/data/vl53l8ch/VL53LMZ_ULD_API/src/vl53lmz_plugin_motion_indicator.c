...
#define MI_SFE_STANDARD_MODE		0x00
		...
		uint8_t				resolution)
{
	...
	p_motion_config->cnh_cfg = MI_SFE_STANDARD_MODE;
	p_motion_config->cnh_flex_shift = 0;
	p_motion_config->spare_3 = 0;

	status |= vl53lmz_motion_indicator_set_resolution(p_dev,
		...
		uint16_t			distance_max_mm)
{
	...
	else
	{
		...
		status |= vl53lmz_dci_write_data(p_dev,
					...
					VL53LMZ_DCI_MOTION_DETECTOR_CFG,
...
uint8_t vl53lmz_motion_indicator_set_resolution(
		...
		uint8_t			resolution)
{
	uint8_t i, status = VL53LMZ_STATUS_OK;

	switch(resolution)
	{
		case VL53LMZ_RESOLUTION_4X4:
			for(i = 0; i < (uint8_t)VL53LMZ_RESOLUTION_4X4; i++)
			{
		...
		case VL53LMZ_RESOLUTION_8X8:
			for(i = 0; i < (uint8_t)VL53LMZ_RESOLUTION_8X8; i++)
			{
