...
// _read() and _write() copied from STM32 syscalls.c file (which Cube.MX insists
// on removing from project).
...
void Reset_Sensor_via_GPIO(void) {
  ...
  // If I2C master does not support "I2C bus clear" feature, need to toggle
  // I2C_SPI_N pin to release I2C bus
